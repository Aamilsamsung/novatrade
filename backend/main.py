...
def token(uid):
    return jwt.encode({'sub': uid, 'exp': datetime.now(timezone.utc) + timedelta(days=1)}, SECRET, algorithm='HS256')
...
def uid(auth: Optional[str] = Header(None)):
    ...
    try:
        return int(jwt.decode(auth.split(' ', 1)[1], SECRET, algorithms=['HS256'])['sub'])
    except Exception:
...
def px(s):
    ...
    base = STOCKS[s][1]
    move = ((int(time.time()) // 5) % 21 - 10) / 1000
...
def send_email(to, subject, html):
    from_addr = os.getenv('EMAIL_FROM', 'NovaTrade <onboarding@resend.dev>')
...
def create_auth_token(c, user_id, kind, minutes=60):
    ...
    c.execute('INSERT INTO auth_tokens(user_id,token_hash,kind,expires_at,created_at) VALUES(?,?,?,?,?)', (user_id, hash_token(raw), kind, (datetime.now(timezone.utc) + timedelta(minutes=minutes)).isoformat(), datetime.now(timezone.utc).isoformat()))
    return raw
...
def consume_auth_token(c, raw, kind):
    ...
    c.execute('UPDATE auth_tokens SET used=1 WHERE id=?', (r['id'],))
    return r['user_id']
...
def signup(a: Auth):
    ...
    try:
        cur = c.execute('INSERT INTO users(email,password,name,created_at,verified) VALUES(?,?,?,?,0)', (email, pwd.hash(a.password), a.name.strip() or 'Trader', datetime.now(timezone.utc).isoformat()))
        u = cur.lastrowid
        c.execute('INSERT INTO portfolios VALUES(?,?,?)', (u, 100000, 100000)); c.execute('INSERT INTO bots VALUES(?,?,?,?,?,?)', (u, 0, 'RELIANCE', 1, 2, 4))
        if not email_configured():
            c.rollback(); raise HTTPException(503, 'Email service is not configured yet. Add RESEND_API_KEY or SMTP settings in Render.')
        raw = create_auth_token(c, u, 'verify', 1440); c.commit()
        link = f'{BASE_URL}/?verify={raw}'
        try:
            send_email(email, 'Confirm your NovaTrade account', f'<div style="font-family:Arial"><h2>Welcome to NovaTrade</h2><p>Confirm your email to activate your account.</p><p><a href="{link}" style="background:#08a66b;color:white;padding:12px 18px;border-radius:8px;text-decoration:none">Confirm email</a></p><p>This link expires in 24 hours.</p></div>')
        except Exception:
